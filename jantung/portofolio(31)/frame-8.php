<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="initial-scale=1, width=device-width" />

  <link rel="stylesheet" href="./global.css" />
  <link rel="stylesheet" href="./frame-8.css" />
  <link
    rel="stylesheet"
    href="https://fonts.googleapis.com/css2?family=Poppins:ital,wght@0,200;0,300;0,400;0,500;0,600;0,700;0,800;0,900;1,500&display=swap"
  />
  <link
    rel="stylesheet"
    href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;600;800;900&display=swap"
  />
  <link
    rel="stylesheet"
    href="https://fonts.googleapis.com/css2?family=Oxygen:wght@300;700&display=swap"
  />
  <link rel="stylesheet" href="./styles.css" />
  <style>
      /* CSS untuk mempertahankan desain tombol "Buy Now" */
      .buy-now-wrapper1 {
        display: inline-block;
      }

      .buy-now6 {
        background-color: var(--color-seagreen); /* Warna latar belakang */
        border: none; /* Tanpa border */
        color: white; /* Warna teks */
        text-align: center; /* Teks rata tengah */
        text-decoration: none; /* Tanpa underline */
        display: inline-block; /* Display inline */
        font-size: 18px; /* Ukuran font */
        cursor: pointer; /* Pointer cursor */
        border-radius: 10px; /* Border radius */
        transition-duration: 0.1; /* Durasi transisi */
        font-weight: bold;
        font-family: var( --font-poppins);
      }

   
    </style>
</head>
<body>
  <div class="rectangle-parent34">
    <div class="frame-child13"></div>
    <div class="header9">
      <div class="header-child7"></div>
      <div class="portofolio19">
        <span>Porto</span>
        <span class="folio12">folio.</span>
      </div>
      <a href="../../hati/payment/index.php" class="home10">HOME</a>
      <a href="../../hati/portofolio(35)/about-us.php" class="about16">ABOUT</a>
      <a href="#" class="login14">LOGIN</a>
      <a href="../../hati/portofolio(13)/desktop-1.php" class="membership10">MEMBERSHIP</a>
    </div>
    <b class="cloud-painting">Eye Expression</b>
    <div class="lorem-ipsum-dolor9">
      Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut eleifend
      nunc id accumsan posuere. Quisque sed ante a risus eleifend elementum a
      sit amet est. Donec eros est.
    </div>
    <div class="frame-child14"></div>
    <div class="rp200003">Rp.800.000</div>
    
    <!-- Form untuk submit dengan method POST -->
    <form action="process_payment.php" method="post">
      <div class="buy-now-wrapper1">
        <!-- Tombol Submit -->
        <button type="submit" class="buy-now6">Buy Now</button>
      </div>
    </form>
    
    <a href="../portofolio(32)/frame-8.php" class="chevron-right-fill0-wght400-gr-icon7">
      <img
        alt=""
        src="./public/chevron-right-fill0-wght400-grad0-opsz24-11.svg"
      />
    </a>

    <a href="../portofolio(30)/frame-8.php" class="chevron-right-fill0-wght400-gr-icon8">
      <img
        alt=""
        src="./public/chevron-right-fill0-wght400-grad0-opsz24-2.svg"
      />
    </a>

    <img class="img6-1-icon" alt="" src="./public/img6-1@2x.png" />
  </div>
</body>
</html>
